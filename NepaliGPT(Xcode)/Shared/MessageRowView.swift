    ...
    var body: some View {
        VStack(spacing: 0) {
            messageRow(text: message.sendText, image: message.sendImage,
                       bgColor: colorScheme == .light ? .white : Color(red: 24/255, green: 107/255, blue: 240/255, opacity: 0.5))
            
            if let text = message.responseText {
                
                messageRow(text: text, image: message.responseImage, bgColor: colorScheme == .light ? .blue.opacity(0.8) : Color(red: 24/255, green: 107/255, blue: 240/255, opacity: 1), responseError: message.responseError, showDotLoading: message.isInteractingWithChatGPT)
                    .cornerRadius(20)
